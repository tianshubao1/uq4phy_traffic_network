...
def quantile_loss(y_pred, y_true):
    ...
    losses = []
    for i, q in enumerate(quantiles):       # 3 quantile values
...
def underwood_loss(y_pred, x_true, y_true): #x_true[0] is normalized, x_true[1], x_true[2] not normalized, [speed, occupy, flow]
    ...
    v_f = np.load('data/sensor_graph/PEMSD4_v_f.npy')   # [1,1,170]
    rho_max = np.load('data/sensor_graph/PEMSD4_rho_max.npy')   # [1,1,170]
    v_f = torch.from_numpy(v_f).to(device)
    rho_max = torch.from_numpy(rho_max).to(device)
    #v_f = 73.379        #70.86
    #rho_max = 0.4365    #0.649
    #y_pred: torch.Size([12, 128, 170, 3])  # 3 quantile number
    y_pred_mid = torch.unbind(y_pred,3)[1]  #y_pred: torch.Size([12, 128, 170])
    x_true = x_true.view(12, 256, 307, 3)   #torch.Size([12, 128, 170, 3])          x_true speed occupancy is normalized,flow is not normalized
    #print(x_true.size())
