...
class DCRNNSupervisor:
    ...
    def evaluate_pgd(self, dataset='val', batches_seen=0,
        ...
        for _, (x, y) in enumerate(val_iterator):
            x, y = self._prepare_data(x, y)   # 与训练/模型输入保持一致
            x0 = x.detach()
            ...
            for _ in range(steps):
                ...
                out_gen = self.dcrnn_model(x_adv)        # 与你FGSM版本一致的前向
                loss_gen = self.compute_mse(out_gen, y)   # 若需完全一致可换成 self._compute_loss(out_gen, y)
                loss_gen.backward()

                with torch.no_grad():
                    grad_sign = x_adv.grad.sign()
                    x_adv = x_adv + alpha * grad_sign              # 梯度上升一步
                    # 投影回以 x0 为中心、半径 epsilon 的 L∞ 球
        ...
        # ---- 拼接到 batch 维 ----
        y_preds  = np.concatenate([t.numpy() for t in y_preds],  axis=1)
    ...
    def _train(self, base_lr,
        ...
        # ---- PGD 训练/评估超参（kwargs 可覆盖；兼容旧 key adv_use_fgsm）----
        adv_use_pgd      = kwargs.get('adv_use_pgd', kwargs.get('adv_use_fgsm', True))  # 是否启用训练期 PGD
        adv_eps          = kwargs.get('adv_eps', 0.03)          # 训练期 PGD 扰动半径 (L∞)
        adv_steps        = kwargs.get('adv_steps', 2)           # ← 默认 2 步
        adv_alpha        = kwargs.get('adv_alpha', None)        # 每步步长，默认 eps/steps
        adv_random_start = kwargs.get('adv_random_start', True) # 是否随机起点
        adv_clip_min     = kwargs.get('adv_clip_min', None)     # 如有物理边界可设 0.0
        adv_clip_max     = kwargs.get('adv_clip_max', None)     # 如有物理边界可设 1.0

        # 评估阶段（可与训练不同；同样默认 2 步）
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与训练一致：_compute_loss(output, y, self.reg, input)
                        loss_gen = self._compute_loss(out_gen, y, self.reg, x_adv)
                        loss_gen.backward()  # 仅为了拿 ∇x

                        with torch.no_grad():
                            grad_sign = x_adv.grad.sign()
                            x_adv = x_adv + adv_alpha * grad_sign
                            # 投影回 L∞ ball: x0 - eps <= x_adv <= x0 + eps
                ...
                # ------- 2) 真正训练步：仅用 x_adv -------
                optimizer.zero_grad()  # 清参数梯度，避免生成阶段梯度污染
                ...
                loss = self._compute_loss(output, y, self.reg, x_adv)
    ...
    def _compute_loss(self, y_pred,y_true, reg, x_true):
        y_pred = self.standard_scaler.inverse_transform(y_pred)
        y_true = self.standard_scaler.inverse_transform(y_true)
        return mis_loss(y_pred, y_true)+ reg* underwood_loss(y_pred, x_true, y_true)
