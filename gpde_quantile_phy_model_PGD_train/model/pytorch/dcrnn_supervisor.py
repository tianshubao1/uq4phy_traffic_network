...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        self.reg = float(self._model_kwargs.get('reg', 0.1))  # for the physical regularization
    ...
    def evaluate_pgd(self, dataset='val', batches_seen=0,
        ...
        for _, (x, y) in enumerate(it):
            ...
            for _ in range(steps):
                ...
                out_gen = self.dcrnn_model(x_adv)         # 与原 FGSM 版本一致：仅传 x_adv
    ...
    def _train(self, base_lr,
        ...
        # ---- PGD 训练超参（可通过 kwargs 覆盖；兼容旧 key）----
        ...
        adv_clip_min     = kwargs.get('adv_clip_min', None)     # 有物理边界就填 0.0
        adv_clip_max     = kwargs.get('adv_clip_max', None)     # 有物理边界就填 1.0

        # 评估期 PGD 参数（可与训练不同；保留原 fgsm_* 兼容）
        pgd_eps_eval     = kwargs.get('pgd_eps_eval', kwargs.get('fgsm_eps_eval', adv_eps))
        pgd_steps_eval   = kwargs.get('pgd_steps_eval', adv_steps)
        pgd_alpha_eval   = kwargs.get('pgd_alpha_eval', None)   # None → eps/steps
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与你的训练一致：_compute_loss(output, y, self.reg, input)
