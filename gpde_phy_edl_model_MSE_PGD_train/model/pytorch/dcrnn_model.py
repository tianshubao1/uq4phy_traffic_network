...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.v_f = 54.45        #METR-LA free-speed
        self.rho_max = 680    #METR-LA, rho_max,  this hyperparameter needs to be carefully chosen
        self.delta_t = 300/3600     # 5 min        
        
        LA_matrix = np.load('data/sensor_graph/LA_matrix.npy')            
        self.weight_matrix = torch.from_numpy(LA_matrix).to(device)        
    ...
    def forward(self, inputs, batch, hidden_state=None):
        ...
        #all these tensor size is for METR-LA
        ...
        batch = batch.view(self.batch_size, size1, self.num_nodes)  # batch: (128, 3, 170)
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.projection_layer_gamma = nn.Linear(self.rnn_units, self.output_dim)
        self.projection_layer_nu = nn.Linear(self.rnn_units, self.output_dim)
        self.projection_layer_alpha = nn.Linear(self.rnn_units, self.output_dim)
        self.projection_layer_beta = nn.Linear(self.rnn_units, self.output_dim)
        ...
        self.batch_size = 256 #128
        self.v_f = 54.45
        self.rho_max = 680.62    #METR-LA, rho_max,  this hyperparameter needs to be carefully chosen
        self.delta_t = 300/3600     # 5 min          
        
        LA_matrix = np.load('data/sensor_graph/LA_matrix.npy')  
        #self.pemsd8_distance = torch.from_numpy(pemsd8)/1609.344  #100       #meters to mile
        #self.pemsd8_distance = torch.exp(self.pemsd8_distance)
        #self.weight_matrix = 1/self.pemsd8_distance   
        self.weight_matrix = torch.from_numpy(LA_matrix).to(device)              
        self.weight_matrix[self.weight_matrix < 0.005] = 0       
        self.source_matrix = torch.transpose(self.weight_matrix, 0, 1)
    ...
    def forward(self, inputs, batch, hidden_state=None):
        ...
        #all these tensor size is for PEMSD8
        #batch: torch.Size([128, 510])
        #print(inputs.size())   torch.Size([128, 170])
        size1 = inputs.size()[1]/ self.num_nodes  #1 
        size1 = int(size1)
        size2 = batch.size()[1]/ self.num_nodes  #2
        size2 = int(size2)
        #print(size2)
        x_input = inputs.view(self.batch_size, size1, self.num_nodes)       #    [128, 1, 170]       
        
        if self.h_input is None:       
            self.h_input = nn.Parameter(torch.FloatTensor(1, size1, self.num_nodes).to(device))      # self.h: (2, 170)
            init.normal_(self.h_input, mean=0.0, std=0.1)  
        
        # ----------------------------- x_input ----------------------------------
        
        batch = batch.view(self.batch_size, size2, self.num_nodes)  # batch: (128, 3, 170)
        rho_input = batch[:, 1, :]        # occupancy   rho_input:(128, 170)
        #print(rho_input)
        rho_input = rho_input.view(self.batch_size, 1, self.num_nodes)    #rho_input:(128, 1, 170)
        broad_input = torch.zeros(self.batch_size, size1, self.num_nodes).to(device)   # rho_input:(128, size1, 170)
        rho_input = rho_input + broad_input     #rho_input:(128, 2, 207)
        ...
        inputs = x_input_next.view(self.batch_size, 3 *self.num_nodes)        
        ...
        projected_gamma = self.projection_layer_gamma(output.view(-1, self.rnn_units))
        gamma =projected_gamma.view(-1, self.num_nodes * self.output_dim)
        
        projected_nu = self.projection_layer_nu(output.view(-1, self.rnn_units))
        nu = nn.Softplus()(projected_nu.view(-1, self.num_nodes * self.output_dim))
        
        projected_alpha = self.projection_layer_alpha(output.view(-1, self.rnn_units))
        alpha = nn.Softplus()(projected_alpha.view(-1, self.num_nodes * self.output_dim)) +1
        
        projected_beta = self.projection_layer_beta(output.view(-1, self.rnn_units))
        beta = nn.Softplus()(projected_beta.view(-1, self.num_nodes * self.output_dim))

        projected = self.projection_layer(output.view(-1, self.rnn_units))
        output = projected.view(-1, self.num_nodes * self.output_dim,3)

        return gamma, nu, alpha, beta, output , torch.stack(hidden_states)
...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    ...
    def decoder(self, encoder_hidden_state, batch, labels=None, batches_seen=None):
        ...
        go_symbol = torch.zeros((batch_size, self.num_nodes * self.decoder_model.output_dim*3),
                                device=device)
        ...
        for t in range(self.decoder_model.horizon):
            gamma_output, nu_output, alpha_output, beta_output,decoder_output, decoder_hidden_state = self.decoder_model(decoder_input, batch[t, :, :],decoder_hidden_state)
            decoder_input = decoder_output.T[1].T
            gamma.append(gamma_output)
            nu.append(nu_output)
            alpha.append(alpha_output)
            beta.append(beta_output)
    ...
    def forward(self, inputs, labels=None, batches_seen=None):
        ...
        gamma, nu, alpha, beta = self.decoder(encoder_hidden_state, inputs_original, labels, batches_seen=batches_seen)
