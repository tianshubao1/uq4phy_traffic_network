...
def underwood_loss(y_pred, x_true, y_true): #x_true[0] is normalized, x_true[1], x_true[2] not normalized, [speed, occupy, flow]
    ...
    #y_pred: torch.Size([12, 128, 207])  # 3 quantile number

    y_pred_mid = y_pred  #y_pred: torch.Size([12, 128, 207])
    #print(x_true.size())
    x_true = x_true.view(12, batch, 207, 2)   #torch.Size([12, 128, 207, 2])          x_true speed occupancy is normalized,flow is not normalized
    occupancy = torch.unbind(x_true,3)[1]   #torch.Size([12, 128, 207])
    #print(occupancy.size())
    v_f = v_f[:, :batch, :] #flow is not normalized, just change the size of v_f to match the size of occupancy
    rho_max = rho_max[:, :batch, :]
    mask = (y_true != 0).float()
    mask /= mask.mean()

    #is_equal = all(torch.equal(rho_max[:, i, :], rho_max[:, 0, :]) for i in range(128))
    #print("The 128 data is the same or not:", is_equal)
