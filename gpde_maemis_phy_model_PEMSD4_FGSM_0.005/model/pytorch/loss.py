...
def underwood_loss(y_pred, x_true, y_true): #x_true[0] is normalized, x_true[1], x_true[2] not normalized, [speed, occupy, flow]
    ...
    batch = 256
    v_f = np.load('data/sensor_graph/PEMSD4_v_f.npy')   # [1,1,307]
    rho_max = np.load('data/sensor_graph/PEMSD4_rho_max.npy')   # [1,1,307]
    v_f = torch.from_numpy(v_f).to(device)
    rho_max = torch.from_numpy(rho_max).to(device)
    y_pred_mid = torch.unbind(y_pred,3)[1]  #y_pred: torch.Size([12, 256, 307])
    x_true = x_true.view(12, batch, 307, 3)   #torch.Size([12, 256, 307, 3])          x_true speed occupancy is normalized,flow is not normalized
    occupancy = torch.unbind(x_true,3)[1]   #torch.Size([12, 256, 307])
    mask = (y_true != 0).float()
    mask /= mask.mean()
