...
class DCRNNSupervisor:
    ...
    def evaluate_fgsm(self, dataset='val', batches_seen=0,
        ...
        for _, (x, y) in enumerate(it):
            ...
            # 1) 生成 FGSM 对抗样本（基于 MSE 攻击）
            x_adv = x.clone().detach().requires_grad_(True)
            self.dcrnn_model.zero_grad()
            out_gen = self.dcrnn_model(x_adv)          # 前向（带梯度）
            loss_gen = self.compute_mse(out_gen, y)    # 若想与训练完全一致，可换成 self._compute_loss(out_gen, y)
            loss_gen.backward()
