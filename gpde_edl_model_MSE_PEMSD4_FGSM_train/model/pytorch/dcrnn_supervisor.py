...
class DCRNNSupervisor:
    ...
    def _train(self, base_lr,
        ...
        adv_eps       = kwargs.get('adv_eps', 0.03)          # 扰动强度
        adv_clip_min  = kwargs.get('adv_clip_min', None)     # 如有物理边界（0.0）
        adv_clip_max  = kwargs.get('adv_clip_max', None)     # 如有物理边界（1.0）
        adv_mix_clean = kwargs.get('adv_mix_clean', False)   # True: clean+adv 混合；False: 仅 adv
        adv_lambda    = kwargs.get('adv_lambda', 0.5)        # clean 权重，仅在混合时使用
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            # ------- FGSM 验证集 -------
            fgsm_eps = kwargs.get('fgsm_eps_eval', adv_eps)  # 评估用 eps，可与训练不同
