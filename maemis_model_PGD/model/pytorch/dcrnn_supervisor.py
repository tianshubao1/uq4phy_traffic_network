...
class DCRNNSupervisor:
    ...
    def evaluate_pgd(self, dataset='val', batches_seen=0,
        ...
        """
        PGD(L∞) 对抗评估（默认 2 步）
        返回: loss_mis, loss_width, loss_mse, loss_rmse, loss_mae, {
        ...
        for _, (x, y) in enumerate(val_iterator):
            ...
            for _ in range(steps):
                ...
                loss_gen = self.compute_mse(out_gen, y) # 如需完全一致可换 self._compute_loss(out_gen, y)
                loss_gen.backward()
    ...
    def _train(self, base_lr,
        ...
        # ===== PGD(ℓ∞) 训练超参 =====
        adv_use_pgd      = kwargs.get('adv_use_pgd', True)   # 是否启用 PGD 对抗训练
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与你的训练一致：_compute_loss(output, y)
                        loss_gen = self._compute_loss(out_gen, y)
                        loss_gen.backward()  # 仅为拿 ∇x
