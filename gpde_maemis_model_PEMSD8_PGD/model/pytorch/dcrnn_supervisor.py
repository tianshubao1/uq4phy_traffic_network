...
class DCRNNSupervisor:
    ...
    def evaluate_pgd(self, dataset='val', batches_seen=0,
        ...
        step_size = (alpha if alpha is not None else epsilon / max(1, steps))
        ...
        for _, (x, y) in enumerate(val_iterator):
            ...
            for _ in range(steps):
                ...
                loss_gen = self.compute_mse(out_gen, y) # 若需一致可换 _compute_loss(out_gen, y)
                loss_gen.backward()

                with torch.no_grad():
                    x_adv = x_adv + step_size * x_adv.grad.sign()          # 梯度上升一步
                    x_adv = torch.max(torch.min(x_adv, x0 + epsilon),      # 投影到 L∞(x0, ε)
                                       x0 - epsilon)
                    if (clip_min is not None) and (clip_max is not None):  # 合法范围裁剪
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与训练一致的损失签名
                        loss_gen = self._compute_loss(out_gen, y)
            ...
            val_mis_adv, val_width_adv, val_mse_adv, val_rmse_adv, val_mae_adv, _ = \
                self.evaluate_pgd(dataset='val',
                                  ...
                                  epsilon=pgd_eps_eval, steps=pgd_steps_eval, alpha=eval_alpha,
