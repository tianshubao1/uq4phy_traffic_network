...
class DCRNNSupervisor:
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            print("data mean: ",self.standard_scaler.mean)
            print("data std: ",self.standard_scaler.std)
    ...
    def evaluate_pgd(self, dataset='test', batches_seen=0,
        ...
        steps = 2
        step_size = epsilon / steps  # 两步 PGD 默认每步 eps/2
        ...
        for _, (x, y) in enumerate(val_iterator):
            ...
            # ------- PGD 初始化（无随机起点；如需随机可在此加 random_start）-------
            x0 = x.detach()
            x_adv = x0.clone()
        ...
        results = {
            ...
            # 保留键名 'fgsm' 以兼容下游，但注明 attack='pgd'
            'fgsm': {
                'attack': 'pgd',
                'eps': float(epsilon),
                'steps': int(steps),
    ...
    def _train(self, base_lr,
        ...
        # ===== PGD(ℓ∞) 训练超参（由原 FGSM 改为 PGD）=====
        adv_use_pgd     = kwargs.get('adv_use_pgd', True)      # 是否开启训练期 PGD
        adv_eps         = kwargs.get('adv_eps', 0.03)          # 扰动半径 ε
        adv_steps       = kwargs.get('adv_steps', 2)           # PGD 步数（默认 2 步）
        ...
        # 评估阶段仍用你的 evaluate_fgsm（如已改成 evaluate_pgd，可相应替换）
        fgsm_eps = kwargs.get('fgsm_eps_eval', adv_eps)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与你的训练一致的损失签名
                        loss_gen = self._compute_loss(g_gen, n_gen, a_gen, b_gen, y)
                ...
                if adv_use_pgd and adv_mix_clean:
                    ...
                    # 仅对抗样本（或未启用 PGD 时等同 clean）
                    gamma, nu, alpha, beta = self.dcrnn_model(x_adv, y, batches_seen)
            ...
            # ------- 验证（若你已实现 evaluate_pgd，可把下面两处换成 evaluate_pgd）-------
