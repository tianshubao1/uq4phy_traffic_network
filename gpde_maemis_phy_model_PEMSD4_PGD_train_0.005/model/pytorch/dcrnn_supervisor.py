...
class DCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        self.reg = float(self._model_kwargs.get('reg', 0.01))  # for the physical regularization
    ...
    def evaluate_pgd(self, dataset='val', batches_seen=0,
                     epsilon=0.03, steps=2, alpha=None,
                     random_start=True, clip_min=None, clip_max=None):

        self.dcrnn_model.eval()
        alpha = epsilon / steps if (alpha is None) else alpha
        ...
        for _, (x, y) in enumerate(val_iterator):
            x, y = self._prepare_data(x, y)   # x: [B, ...]，与模型输入一致
            x0 = x.detach()                   # 原始干净样本的副本，用于投影

            # ---- 初始化 x_adv ----
            if random_start:
                # 在 [-epsilon, +epsilon] 中随机扰动，然后裁剪到合法输入范围
            ...
            for _ in range(steps):
                ...
                out_gen = self.dcrnn_model(x_adv)         # 前向
                loss_gen = self.compute_mse(out_gen, y)   # 也可换成训练时的 _compute_loss
                loss_gen.backward()
                ...
                with torch.no_grad():
                    ...
                    # 投影回 L∞-ball:  x0 - epsilon <= x_adv <= x0 + epsilon
    ...
    def _train(self, base_lr,
        ...
        # ---- PGD 训练期/评估期超参数（kwargs 可覆盖；兼容旧 key adv_use_fgsm）----
        adv_use_pgd     = kwargs.get('adv_use_pgd', kwargs.get('adv_use_fgsm', True))
        adv_eps         = kwargs.get('adv_eps', 0.03)
        adv_steps       = kwargs.get('adv_steps', 2)
        adv_alpha       = kwargs.get('adv_alpha', None)  # 默认: adv_eps / adv_steps
        adv_random_start= kwargs.get('adv_random_start', True)
        adv_clip_min    = kwargs.get('adv_clip_min', None)
        adv_clip_max    = kwargs.get('adv_clip_max', None)

        # 评估阶段（可与训练不同）
        pgd_eps_eval    = kwargs.get('pgd_eps_eval', adv_eps)
        pgd_steps_eval  = kwargs.get('pgd_steps_eval', adv_steps)
        pgd_alpha_eval  = kwargs.get('pgd_alpha_eval', adv_alpha)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                # ------- 1) 生成 x_adv（PGD 内循环，仅取 ∇_x；与训练相同的损失）-------
                if adv_use_pgd:
                    x0 = x.detach()
                    if adv_random_start:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 注意：与训练一致的损失签名（output, y, self.reg, x_adv）
                    ...
                    x_adv = x  # 关闭对抗训练时退化为 clean

                # ------- 2) 真正训练步：仅用 x_adv -------
                optimizer.zero_grad()  # 清参数梯度，避免生成阶段梯度污染

                # 兼容你首个 batch 的优化器/调度器重建逻辑
                if batches_seen == 0:
                    optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=base_lr, eps=epsilon)
