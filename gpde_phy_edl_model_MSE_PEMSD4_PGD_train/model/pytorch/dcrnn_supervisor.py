...
class DCRNNSupervisor:
    ...
    def evaluate_pgd(self, dataset='test', batches_seen=0,
                     epsilon=0.03, steps=2, alpha=None, random_start=True,
                     clip_min=None, clip_max=None, attack_loss='mse'):
        ...
        # 为两种分支准备容器
        single_outputs = []  # 单输出
        gamma_preds, nu_preds, alpha_preds, beta_preds = [], [], [], []  # 四输出

        print("PGD eval | data mean:", self.standard_scaler.mean)
        print("PGD eval | data std :", self.standard_scaler.std)
        ...
        for _, (x, y) in enumerate(val_iterator):
            ...
            for _step in range(steps):
                ...
                try:
                    ...
                    out_gen = self.dcrnn_model(x_adv)

                # 选择攻击损失
                if attack_loss == 'full':
                    loss_gen = self._compute_loss(out_gen, y, self.reg, x_adv)
                else:  # 'mse'
                    if isinstance(out_gen, (tuple, list)) and len(out_gen) == 4:
                        loss_gen = self.compute_mse(out_gen[0], y)  # 用 gamma 与 y 的 MSE
                    else:
                        loss_gen = self.compute_mse(out_gen, y)
            ...
            with torch.no_grad():
                try:
                    out = self.dcrnn_model(x_adv, y, batches_seen)
                except TypeError:
                    out = self.dcrnn_model(x_adv)

                if isinstance(out, (tuple, list)) and len(out) == 4:
                    gamma, nu, alpha_o, beta = out
                    losses_mis.append(self.compute_mis(gamma, nu, alpha_o, beta, y).item())
                    losses_width.append(self.compute_width(gamma, nu, alpha_o, beta, y).item())
                    losses_mse.append(self.compute_mse(gamma, y).item())
                    losses_mae.append(self._compute_mae(gamma, y).item())

                    gamma_preds.append(gamma.cpu())
                    nu_preds.append(nu.cpu())
                    alpha_preds.append(alpha_o.cpu())
                    beta_preds.append(beta.cpu())
                else:
                    output = out
                    ...
                    single_outputs.append(output.cpu())
        ...
        # 单输出
        if len(single_outputs) > 0:
            single_outputs = np.concatenate(single_outputs, axis=1)
            pred_scaled = []
            for t in range(len(single_outputs)):
                pred_scaled.append(self.standard_scaler.inverse_transform(single_outputs[t]))
            results['prediction'] = [pred_scaled]

        # 四输出
        if len(gamma_preds) > 0:
            gamma_preds = np.concatenate(gamma_preds, axis=1)
            nu_preds    = np.concatenate(nu_preds,    axis=1)
            alpha_preds = np.concatenate(alpha_preds, axis=1)
            beta_preds  = np.concatenate(beta_preds,  axis=1)

            gamma_scaled, nu_scaled, alpha_scaled, beta_scaled = [], [], [], []
            for t in range(len(gamma_preds)):
                gamma_scaled.append(self.standard_scaler.inverse_transform(gamma_preds[t]))
                nu_scaled.append(self.standard_scaler.inverse_transform(nu_preds[t]))
                alpha_scaled.append(self.standard_scaler.inverse_transform(alpha_preds[t]))
                beta_scaled.append(self.standard_scaler.inverse_transform(beta_preds[t]))
            results['prediction'] = [gamma_scaled, nu_scaled, alpha_scaled, beta_scaled]
    ...
    def _train(self, base_lr,
        ...
        # ===== PGD 训练与评估的超参数（可通过 kwargs 覆盖）=====
        adv_use_pgd      = kwargs.get('adv_use_pgd', True)         # 训练是否开启 PGD
        adv_eps          = kwargs.get('adv_eps', 0.03)             # L_inf 扰动半径
        adv_steps        = kwargs.get('adv_steps', 2)             # PGD 步数
        adv_alpha        = kwargs.get('adv_alpha', None)           # 每步步长（默认 eps/steps）
        adv_random_start = kwargs.get('adv_random_start', True)    # 是否随机起点（x±eps）
        adv_mix_clean    = kwargs.get('adv_mix_clean', False)      # True: clean+adv 混合；False: 仅 adv
        adv_lambda       = kwargs.get('adv_lambda', 0.5)           # 混合时 clean 权重
        adv_clip_min     = kwargs.get('adv_clip_min', None)        # 物理下界（如 0.0）
        adv_clip_max     = kwargs.get('adv_clip_max', None)        # 物理上界（如 1.0）

        # 评估阶段 PGD 参数（可与训练不同）
        eval_eps          = kwargs.get('eval_eps', adv_eps)
        eval_steps        = kwargs.get('eval_steps', adv_steps)
        eval_alpha        = kwargs.get('eval_alpha', None)
        eval_random_start = kwargs.get('eval_random_start', True)

        if adv_alpha is None:
            adv_alpha = adv_eps / max(1, adv_steps)
        if eval_alpha is None:
            eval_alpha = eval_eps / max(1, eval_steps)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                # ===== 生成 x_adv：PGD（用训练同款四输出损失），仅当开启 PGD =====
                if adv_use_pgd:
                    # init
                    if adv_random_start:
                        x_adv = x.detach() + torch.empty_like(x).uniform_(-adv_eps, adv_eps)
                    else:
                        x_adv = x.detach().clone()
                    ...
                    # PGD 多步
                    for _step in range(adv_steps):
                        x_adv.requires_grad_(True)
                        self.dcrnn_model.zero_grad()

                        out_gen = self.dcrnn_model(x_adv, y, batches_seen)
                        g_gen, n_gen, a_gen, b_gen = out_gen
                        loss_gen = self._compute_loss(g_gen, n_gen, a_gen, b_gen, y, self.reg, x_adv)
                        loss_gen.backward()

                        with torch.no_grad():
                            x_adv = x_adv + adv_alpha * x_adv.grad.sign()                 # 梯度上升
                            x_adv = torch.max(torch.min(x_adv, x + adv_eps), x - adv_eps) # 投影回 ε-球
                            if (adv_clip_min is not None) and (adv_clip_max is not None):
                                x_adv.clamp_(adv_clip_min, adv_clip_max)
                        x_adv = x_adv.detach()
                else:
                ...
                # ===== 真正训练步：清梯度，再算 clean/adv（或混合）损失并反传 =====
                optimizer.zero_grad()
                ...
                if adv_use_pgd and adv_mix_clean and (x_adv is not None):       #not use this
                    # clean
                    out_c = self.dcrnn_model(x, y, batches_seen)
                    g_c, n_c, a_c, b_c = out_c
                    loss_clean = self._compute_loss(g_c, n_c, a_c, b_c, y, self.reg, x)

                    # adv
                    out_a = self.dcrnn_model(x_adv, y, batches_seen)
                    g_a, n_a, a_a, b_a = out_a
                    loss_adv = self._compute_loss(g_a, n_a, a_a, b_a, y, self.reg, x_adv)

                    loss = adv_lambda * loss_clean + (1.0 - adv_lambda) * loss_adv

                elif adv_use_pgd and not adv_mix_clean and (x_adv is not None):
                    # 仅对抗
                    out_a = self.dcrnn_model(x_adv, y, batches_seen)
                    g_a, n_a, a_a, b_a = out_a
                    loss = self._compute_loss(g_a, n_a, a_a, b_a, y, self.reg, x_adv)

                else:
                    # 仅 clean
                    out_c = self.dcrnn_model(x, y, batches_seen)
                    g_c, n_c, a_c, b_c = out_c
                    loss = self._compute_loss(g_c, n_c, a_c, b_c, y, self.reg, x)
            ...
            # ================= Early Stopping（基于 PGD 验证 MAE） =================
            if val_mae_adv < min_val_loss:
                wait = 0
                if save_model:
                    model_file_name = self.save_model(epoch_num)
                    self._logger.info(
                        'Val loss_adv (PGD) decrease from {:.4f} to {:.4f}, saving to {}'.format(
                ...
                if wait >= patience:
                    self._logger.info(
                        f'Early stopping at epoch {epoch_num} (no PGD improvement for {patience} epochs).'
                    )
                    break
    ...
    def _compute_loss(self,gamma, nu, alpha, beta,y_true, reg, x_true): #gamma is y_pred??
