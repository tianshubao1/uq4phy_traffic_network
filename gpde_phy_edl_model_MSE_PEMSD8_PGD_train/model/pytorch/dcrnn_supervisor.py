...
class DCRNNSupervisor:
    ...
    def evaluate_pgd(self, dataset='test', batches_seen=0,
        ...
        for _, (x, y) in enumerate(val_iterator):
            ...
            for _ in range(steps):
                ...
                gamma_gen, nu_gen, alpha_out_gen, beta_gen = self.dcrnn_model(x_adv)

                if attack_loss == 'full':
                    loss_gen = self._compute_loss(gamma_gen, nu_gen, alpha_out_gen, beta_gen, y)
                else:  # 'mse'（默认，仅以 gamma 与 y 的 MSE 攻击）
                    loss_gen = self.compute_mse(gamma_gen, y)

                loss_gen.backward()

                with torch.no_grad():
                    x_adv = x_adv + step_size * x_adv.grad.sign()                # 梯度上升一步
                    x_adv = torch.max(torch.min(x_adv, x0 + epsilon), x0 - epsilon)  # 投影回 L∞-ball
                    if (clip_min is not None) and (clip_max is not None):        # 输入裁剪
                        x_adv.clamp_(clip_min, clip_max)

            # ------- 在 x_adv 上做推理与指标（无梯度） -------
            with torch.no_grad():
                gamma, nu, alpha_out, beta = self.dcrnn_model(x_adv)

                losses_mis.append(self.compute_mis(gamma, nu, alpha_out, beta, y).item())
                losses_width.append(self.compute_width(gamma, nu, alpha_out, beta, y).item())
                losses_mse.append(self.compute_mse(gamma, y).item())
                losses_mae.append(self._compute_mae(gamma, y).item())
                ...
                gamma_preds.append(gamma.cpu())
                nu_preds.append(nu.cpu())
                alpha_preds.append(alpha_out.cpu())
                beta_preds.append(beta.cpu())
        ...
        # 拼接 batch 维（转 numpy 更稳妥）
        gamma_preds = np.concatenate([t.numpy() for t in gamma_preds], axis=1)
        nu_preds    = np.concatenate([t.numpy() for t in nu_preds],    axis=1)
        alpha_preds = np.concatenate([t.numpy() for t in alpha_preds], axis=1)
        beta_preds  = np.concatenate([t.numpy() for t in beta_preds],  axis=1)
        y_truths    = np.concatenate([t.numpy() for t in y_truths],    axis=1)
    ...
    def _train(self, base_lr,
        ...
        # ===== PGD 对抗训练开关与超参数（从 kwargs 读取，可覆盖默认值）=====
        adv_use_pgd      = kwargs.get('adv_use_pgd', kwargs.get('adv_use_fgsm', True))  # 兼容旧key
        adv_eps          = kwargs.get('adv_eps', 0.03)       # 扰动半径（按数据尺度调整）
        adv_steps        = kwargs.get('adv_steps', 2)        # ← 默认 2 步
        adv_alpha        = kwargs.get('adv_alpha', None)     # 每步步长，默认 eps/steps
        adv_random_start = kwargs.get('adv_random_start', True)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                if adv_use_pgd:
                    ...
                    for _ in range(adv_steps):
                        ...
                        # 与你的训练一致：_compute_loss(g, n, a, b, y, self.reg, x_adv)
                ...
                if adv_use_pgd and adv_mix_clean:
                    # clean 前向与损失
                ...
                elif adv_use_pgd and not adv_mix_clean:
                    # 仅对抗样本训练
                    gamma_a, nu_a, alpha_a, beta_a = self.dcrnn_model(x_adv, y, batches_seen)
            ...
            # ================= 验证/测试（PGD 对抗评估）=================
            # 评估阶段的 PGD 参数（允许不同于训练）
            pgd_eps_eval    = kwargs.get('pgd_eps_eval', kwargs.get('fgsm_eps_eval', adv_eps))
            pgd_steps_eval  = kwargs.get('pgd_steps_eval', adv_steps)
            pgd_alpha_eval  = kwargs.get('pgd_alpha_eval', None if adv_alpha is None else adv_alpha)
            pgd_random_eval = kwargs.get('pgd_random_eval', True)
            if pgd_alpha_eval is None:
                pgd_alpha_eval = pgd_eps_eval / max(1, pgd_steps_eval)
            ...
            val_mis_adv, val_width_adv, val_mse_adv, val_rmse_adv, val_mae_adv, _ = \
                self.evaluate_pgd(dataset='val',
                                  ...
                                  random_start=pgd_random_eval,
                                  clip_min=adv_clip_min, clip_max=adv_clip_max)

            self._logger.info(
                f"Epoch [{epoch_num}/{epochs}] ({batches_seen}) "
                f"[PGD eps={pgd_eps_eval}, steps={pgd_steps_eval}, alpha={pgd_alpha_eval:.6f}, rand={pgd_random_eval}] "
