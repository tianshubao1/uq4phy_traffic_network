...
class DCRNNSupervisor:
    ...
    def load_model(self):
        self._setup_graph()
        assert os.path.exists('dcrnn_edl_model_MSE_PEMSD8/models/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('dcrnn_edl_model_MSE_PEMSD8/models/epo%d.tar' % self._epoch_num, map_location='cpu')
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            print("mean: ",self.standard_scaler.mean)
            print("std: ",self.standard_scaler.std)
            for _, (x, y) in enumerate(val_iterator):
                x, y = self._prepare_data(x, y)
                #print("num = ", _ )
