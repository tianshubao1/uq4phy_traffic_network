...
class DCRNNSupervisor:
    ...
    def _train(self, base_lr,
        ...
        # ===== PGD(ℓ∞) 训练参数（默认 2 步）=====
        adv_eps          = kwargs.get('adv_eps', 0.03)        # 扰动半径
        adv_steps        = kwargs.get('adv_steps', 2)         # PGD 步数（两步）
        adv_alpha        = kwargs.get('adv_alpha', None)      # 步长，默认 eps/steps
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                for _ in range(adv_steps):
                    ...
                    gamma_gen, nu_gen, alpha_gen, beta_gen = self.dcrnn_model(x_adv, y, batches_seen)
                    # 与你训练一致的损失
                    loss_gen = self._compute_loss(gamma_gen, nu_gen, alpha_gen, beta_gen, y)
                    loss_gen.backward()  # 仅为拿 ∇_x
            ...
            # ------- （保持你现有的 FGSM 评估不变，如需 PGD 评估可改为 evaluate_pgd）-------
            fgsm_eps = 0.03
